...
const promise = async ({ currentDepth, depth, doc, field, fieldPromises, findMany, flattenLocales, overrideAccess, populationPromises, req, siblingDoc, showHiddenFields, context, }) => {
    ...
    if ((0, types_1.fieldAffectsData)(field)) {
        ...
        if (field.access && field.access.read) {
            const result = overrideAccess ? true : await field.access.read({ req, id: doc.id, siblingData: siblingDoc, data: doc, doc: doc }); // TODO: This can be better typed. Might need to add or omit id from FieldWithID somewhere
            if (!result) {
                delete siblingDoc[field.name];
            }
        }
